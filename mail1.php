...
if(!$conn){
	...
	if($row)
	{
		if($row['username']==$x)
		{
			...
			$mail->addAddress($x);
			$mail->isHTML(true);
			$output='congratulations! '.$row['username'].'your '.$row['title'].' is very impressive.';
                            //replace the site url
                            $output.='you are shortlisted for next process.For more details staytuned with ASIIM';
			$mail->Body = $output;
	    }	
		if($mail->send()){

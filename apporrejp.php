 ...
 <!--<div class="Nav"> 
    ...
    <div class="navbar-container container">
        ...
        <ul class="menu-items">
            ...
            <li><a href="1.php">FOR APPROVAL</a></li>
        </ul>
      ...
      <center><form method="post" action="" name="expertsview" enctype="multipart/form-data">
        <table>
           <tr>
            ...
            You can send mails only if all the experts have verified users under their theme.</3></center></td>
            </tr> 
            <tr>
...
$sql_query="SELECT * FROM lap";
$sql_query1="SELECT * FROM lar";
$sql_query2="SELECT * FROM dumprototype";
$sql_query3="SELECT * FROM prototype";
...
foreach ($address as $gui) {
    $mail->addAddress($gui);
    $output='Hi! ur idea is impressive';
			$mail->Body = $output;
    $mail->AddCC =($gui);
    if (!$mail->send()) {
...
foreach ($address1 as $gui) {
    $mail->addAddress($gui);
    $output='Hi!'.$gui.'your prototype is not upto the mark . <br>Any issues contact through mail sihfeistycoders@gmail.com';
			$mail->Body = $output;
    $mail->AddCC =($gui);
    if (!$mail->send()) {
        echo 'Mailer Error: ' . $mail->ErrorInfo;
